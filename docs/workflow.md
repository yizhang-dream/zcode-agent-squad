...
- 独立任务必须拆开同时派：多文件分头改、多模块分头搜、多命令分头蹲；常规批量 5-15 个，多个派发放在同一条消息里并发发出（并发容量按你的套餐上限渲染进规则，默认 50）。
- **同一文件的改动归同一个子 agent**——两个 agent 并发 Edit 同一文件必然冲突。
- 派发指令各自自包含（子 agent 之间看不见彼此）。
